...
# 	For example, while dragging an item on screen a selector can track both what the item is and the cursor's relative position to it.
# @note Stack-based functionality is experimental so it has been omitted. it can be added manually.
class Selector
	extend Identifiable
	include Identity
